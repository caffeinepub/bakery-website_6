...
> = {
  ...
  [Category.cakes]: {
    ...
    colorClass:
      "bg-gaming-purple/20 text-gaming-purple border-gaming-purple/30",
  },
  [Category.drinks]: {
    label: "Drinks",
...
export default function MenuItemCard({ item }: MenuItemCardProps) {
  const catKey =
    Object.values(Category).find((v) => v === item.category) ?? Category.breads;
    ...
    <div className="bg-card rounded-lg overflow-hidden shadow-panel hover:shadow-panel-lg transition-all duration-300 flex flex-col group border border-border hover:neon-border-blue hover:-translate-y-1">
      ...
      <div className="relative h-44 bg-secondary overflow-hidden">
        ...
        <div className="absolute top-2 left-2">
          <span
            className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-body font-bold border ${config.colorClass}`}
          >
            {config.icon}
            {config.label}
          </span>
      ...
      <div className="p-4 flex flex-col flex-1">
        <h3 className="font-display text-base font-bold text-foreground mb-1 tracking-wide">
          {item.name}
        </h3>
        {item.description && (
          <p className="font-body text-sm text-muted-foreground leading-relaxed flex-1 font-medium">
            {item.description}
          </p>
        )}
