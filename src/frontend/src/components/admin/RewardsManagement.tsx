    ...
    <div className="space-y-8">
      ...
      <div>
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="font-display text-lg font-bold tracking-wide">XP Configuration</h3>
        </div>
        {configLoading ? (
        ...
        <form onSubmit={handleUpdateConfig} className="flex gap-3 items-end">
          <div className="space-y-1.5 flex-1 max-w-xs">
            <Label htmlFor="threshold" className="font-body font-semibold tracking-wide">New XP Threshold</Label>
            <Input
              id="threshold"
              type="number"
              min="1"
          ...
          <Button type="submit" disabled={updateConfig.isPending} className="font-body font-bold neon-glow-blue tracking-wide">
      ...
      {/* Record Purchase */}
      <div>
        <h3 className="font-display text-lg font-bold mb-4 tracking-wide">Record XP Purchase</h3>
        <RecordPurchaseForm />
      </div>
      ...
      <div>
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="font-display text-lg font-bold tracking-wide">Player Lookup & Adjust</h3>
        </div>
        <form onSubmit={handleLookup} className="flex gap-3 items-end mb-4">
          <div className="space-y-1.5 flex-1">
            <Label htmlFor="lookup-principal" className="font-body font-semibold tracking-wide">Player Principal ID</Label>
            <Input
              id="lookup-principal"
          ...
          <Button type="submit" disabled={lookupRewards.isPending} variant="outline" className="font-body font-bold gap-1.5 tracking-wide">
        ...
        {foundRewards && (
          <div className="bg-card border neon-border-blue rounded-lg p-4 mt-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h4 className="font-display text-base font-bold tracking-wide">Player Found</h4>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-3 text-sm font-body font-medium">
              <span className="text-muted-foreground">XP Earned: <strong className="text-foreground">{foundRewards.itemsPurchased.toString()}</strong></span>
              <span className="text-muted-foreground">Loot Available: <strong className="text-foreground">{foundRewards.freeTreats.toString()}</strong></span>
            </div>
            <form onSubmit={handleAdjust} className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <Label htmlFor="adj-items" className="font-body text-xs font-semibold tracking-wide">XP Earned</Label>
                  <Input
                    id="adj-items"
                    type="number"
                    min="0"
                ...
                <div className="space-y-1">
                  <Label htmlFor="adj-treats" className="font-body text-xs font-semibold tracking-wide">Loot Available</Label>
                  <Input
                    id="adj-treats"
                    type="number"
                    min="0"
              ...
              <Button type="submit" disabled={adjustRewards.isPending} size="sm" className="font-body font-bold neon-glow-green bg-accent text-accent-foreground hover:bg-accent/90 tracking-wide">
