...
import { Cake, Coffee, Croissant, Wheat } from "lucide-react";
    ...
    <div className="container mx-auto px-4 py-12">
      ...
      <div className="text-center mb-12">
        ...
        <p className="font-body text-muted-foreground max-w-lg mx-auto font-medium">
          All items are crafted in-house with legendary ingredients. Our menu
          rotates seasonally — new loot drops every month.
        </p>
      </div>

      {isLoading && (
        <div className="space-y-12">
          {(["s1", "s2"] as const).map((sk) => (
            <div key={sk}>
              <Skeleton className="h-8 w-40 mb-6 rounded-lg" />
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {(["c1", "c2", "c3"] as const).map((ck) => (
                  <Skeleton key={ck} className="h-64 rounded-lg" />
                ))}
              </div>
            </div>
          ))}
      ...
      {error && (
        <div className="text-center py-16 text-muted-foreground font-body">
          <p className="text-lg font-semibold">
            ⚠ Connection Lost. Please try again shortly.
          </p>
        </div>
      )}
      ...
      {!isLoading && !error && !hasItems && (
        <div className="text-center py-20">
          <p className="font-pixel text-sm text-muted-foreground mb-4 neon-text-blue">
            LOADING...
          </p>
          <p className="font-display text-2xl text-muted-foreground mb-2 tracking-wide">
            ...
            return (
              <section key={cat}>
                <div className="flex items-center gap-3 mb-6 pb-3 border-b border-border">
                  ...
                  <div>
                    <h2
                      className={`font-display text-2xl font-bold tracking-wide ${config.color}`}
                    >
                      {config.label}
                    </h2>
