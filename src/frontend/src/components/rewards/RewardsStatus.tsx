...
import type { CustomerRewards, RewardsConfig } from "../../backend";
...
export default function RewardsStatus({
  ...
  const progressPercent =
    threshold > 0 ? Math.min((itemsPurchased / threshold) * 100, 100) : 0;
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-card rounded-lg p-5 shadow-panel text-center border neon-border-blue">
          ...
          <p className="font-body text-sm text-muted-foreground mt-1 font-semibold">
            XP Earned
          </p>
        </div>
      ...
      <div className="bg-secondary/50 rounded-lg p-4 border border-border">
        <h4 className="font-display text-sm font-bold mb-2 tracking-wide">
          ⚡ How XP Works
        </h4>
        <p className="font-body text-sm text-muted-foreground font-medium">
          Every time you purchase {threshold} items, you earn 1 free treat. Ask
          our staff to record your XP after each visit!
        </p>
      </div>
    </div>
  );
